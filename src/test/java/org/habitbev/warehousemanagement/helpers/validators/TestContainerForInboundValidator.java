...
public class TestContainerForInboundValidator {

    public static final String WAREHOUSE_1 = "warehouse-1";
    public static final int CAPACITY_TO_INBOUND = 5;
    ...
    public void setUp() throws Exception {
        ...
        containerForInboundValidator = new ContainerForInboundValidator(containerService);
    }

    @Test
    public void test_validate_input_null() {
        Assertions.assertThatExceptionOfType(IllegalArgumentException.class).isThrownBy(() -> containerForInboundValidator.validate(null, new WarehouseValidatedEntities.Builder())).withMessageContaining("inboundIdExistenceValidator.input cannot be null");
        verifyZeroInteractions(containerService);
    }

    @Test
    public void test_validate_builder_null() {
        Assertions.assertThatExceptionOfType(IllegalArgumentException.class).isThrownBy(() -> containerForInboundValidator.validate(WarehouseActionValidationRequest.builder().warehouseAction(WarehouseAction.START_INBOUND).build(), null)).withMessageContaining("warehouseEntityBuilder cannot be null");
        verifyZeroInteractions(containerService);
    }

    @Test
    public void test_validate_containerId_null() {
        WarehouseActionValidationRequest input = WarehouseActionValidationRequest.builder().warehouseAction(WarehouseAction.START_INBOUND).warehouseId(WAREHOUSE_1).capacityToInbound(CAPACITY_TO_INBOUND).skuCode(SKU_CODE).build();
        Assertions.assertThatExceptionOfType(IllegalArgumentException.class).isThrownBy(() -> containerForInboundValidator.validate(input, new WarehouseValidatedEntities.Builder())).withMessageContaining("containerId cannot be blank");
        verifyZeroInteractions(containerService);
    }

    @Test
    public void test_validate_warehouseId_null() {
        WarehouseActionValidationRequest input = WarehouseActionValidationRequest.builder().warehouseAction(WarehouseAction.START_INBOUND).containerId(CONTAINER_1).capacityToInbound(CAPACITY_TO_INBOUND).skuCode(SKU_CODE).build();
        Assertions.assertThatExceptionOfType(IllegalArgumentException.class).isThrownBy(() -> containerForInboundValidator.validate(input, new WarehouseValidatedEntities.Builder())).withMessageContaining("warehouseId cannot be blank");
        verifyZeroInteractions(containerService);
    }

    @Test
    public void test_validate_skuCode_null() {
        WarehouseActionValidationRequest input = WarehouseActionValidationRequest.builder().warehouseAction(WarehouseAction.START_INBOUND).containerId(CONTAINER_1).capacityToInbound(CAPACITY_TO_INBOUND).warehouseId(WAREHOUSE_1).build();
        Assertions.assertThatExceptionOfType(IllegalArgumentException.class).isThrownBy(() -> containerForInboundValidator.validate(input, new WarehouseValidatedEntities.Builder())).withMessageContaining("skuCode cannot be blank");
        verifyZeroInteractions(containerService);
    }
    ...
    public void test_validate_capacityToInbound_null() {
        ...
        Assertions.assertThatExceptionOfType(IllegalArgumentException.class).isThrownBy(() -> containerForInboundValidator.validate(input, new WarehouseValidatedEntities.Builder())).withMessageContaining("capacityToInbound cannot be < 0 or null");
        verifyZeroInteractions(containerService);
    }
    ...
    public void test_validate_success() {
        ...
        WarehouseActionValidationRequest input = WarehouseActionValidationRequest.builder().containerId(CONTAINER_1).warehouseId(WAREHOUSE_1).warehouseAction(WarehouseAction.START_INBOUND)
                .capacityToInbound(CAPACITY_TO_INBOUND).skuCode(SKU_CODE).build();
        ...
        WarehouseValidatedEntities.Builder actualBuilderOutput = containerForInboundValidator.validate(input, warehouseEntityBuilderInput);
    ...
    public void test_validate_inbounding_more_than_max_capacity_exception() {
        WarehouseActionValidationRequest input = WarehouseActionValidationRequest.builder().containerId(CONTAINER_1).warehouseId(WAREHOUSE_1).warehouseAction(WarehouseAction.START_INBOUND)
                .capacityToInbound(9).skuCode(SKU_CODE).build();
        WarehouseValidatedEntities.Builder warehouseEntityBuilderInput = new WarehouseValidatedEntities.Builder().warehouseDTO(WarehouseDTO.builder()
                .id(WAREHOUSE_1).companyId(COMPANY_1).build());
        ContainerDTO containerDTO = new ContainerDTO.Builder().containerId(CONTAINER_1).predefinedCapacity(ImmutableMap.of(SKU_CODE, 10)).warehouseId(WAREHOUSE_1).currentCapacityDetails(ContainerCapacity.builder()
                .currentCapacity(2).warehouseContainerId(CONTAINER_1).containerStatus(new PartiallyFilled()).modifiedTime(EPOCH_MILLI).creationTime(EPOCH_MILLI).build()).build();
        GetContainerRequest getContainerRequest = GetContainerRequest.builder().containerId(CONTAINER_1).warehouseId(WAREHOUSE_1).build();
        when(containerService.getContainer(eq(getContainerRequest))).thenReturn(containerDTO);
        Assertions.assertThatExceptionOfType(WarehouseActionValidationException.class).isThrownBy(() -> containerForInboundValidator.validate(input, warehouseEntityBuilderInput));
        verify(containerService).getContainer(eq(getContainerRequest));
        verifyNoMoreInteractions(containerService);
    }
